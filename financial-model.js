...
// IMPORTANT: Also need the updated get_months_and_flags function
// to ensure HEL term is properly respected
...
// Optional: If you want to add debug code to troubleshoot the issue,
// you can temporarily add this code to log values around the refinance month

/*
// Add this inside the for-loop in calculate_debt_service to debug
const refinanceMonth = updatedDictionary['refinance-mortgage'] ? updatedDictionary['refinance-years']*12 : -1;
if (i >= refinanceMonth - 1 && i <= refinanceMonth + 2) {
    console.log(`Month ${i}:`);
    console.log(`  imBeginningBalance: ${imBeginningBalance}`);
    console.log(`  imRefinance[i]: ${imRefinance[i]}`);
    console.log(`  imNetCashFlow: ${imNetCashFlow}`);
    console.log(`  imEndBalance: ${imEndBalance}`);
    console.log(`  imInterestExpense[i]: ${imInterestExpense[i]}`);
    console.log(`  imPaymentPeriod[i]: ${schedule['imPaymentPeriod'][i]}`);
}
*/
